...
$(document).ready(function () {
  ...
  const chatCompletionFillBtn = document.querySelector(
    ".chat-completions-fill-btn"
  );
...
function updateStatus(status, entry_id) {
  ...
  $.ajax({
    ...
    error: function (data) {
      toastr.error(
        "Something went wrong. Please reload the page and try it again."
      );
    },
  });
  return false;
...
function templateSave(template_id) {
  ...
  document.getElementById("custom_template_button").innerHTML =
...
function templateCustomChatSave(template_id) {
  "use strict";

  document.getElementById("custom_template_button_user").disabled = true;
  document.getElementById("custom_template_button_user").innerHTML =
    magicai_localize.please_wait;

  var formData = new FormData();
  formData.append("template_id", template_id);
  formData.append("name", $("#name_user").val());
  formData.append("short_name", $("#short_name_user").val());
  formData.append("description", $("#description_user").val());
  formData.append("role", $("#role_user").val());
  formData.append("human_name", $("#human_name_user").val());
  formData.append("helps_with", $("#helps_with_user").val());
  formData.append("color", $("#color_user").val());

  if ($("#avatar_user").val() != "undefined") {
    formData.append("avatar", $("#avatar_user").prop("files")[0]);
  }
  ...
  $.ajax({
    type: "post",
    url: "/dashboard/user/openai/chat/save",
    data: formData,
    contentType: false,
    processData: false,
    success: function (data) {
      toastr.success("Chat Template Saved Succesfully.");
      location.href = "/dashboard/user/openai/chat/custom-template";

      document.getElementById("custom_template_button_user").disabled = false;
      document.getElementById("custom_template_button_user").innerHTML = "Save";
    },
    error: function (data) {
      var err = data.responseJSON.errors;
      $.each(err, function (index, value) {
...
function templateAIChatSave(template_id) {
  "use strict";

  document.getElementById("custom_template_button_user_aichat").disabled = true;
  document.getElementById("custom_template_button_user_aichat").innerHTML =
    magicai_localize.please_wait;

  var formData = new FormData();
  formData.append("template_id", template_id);
  formData.append("name", $("#name_user_aichat").val());
  formData.append("short_name", $("#short_name_user_aichat").val());
  formData.append("description", $("#description_user_aichat").val());
  formData.append("role", $("#role_user_aichat").val());
  formData.append("human_name", $("#human_name_user_aichat").val());
  formData.append("helps_with", $("#helps_with_user_aichat").val());
  formData.append("color", $("#color_user_aichat").val());

  if ($("#avatar_user_aichat").val() != "undefined") {
    formData.append("avatar", $("#avatar_user_aichat").prop("files")[0]);
  }
  ...
  $.ajax({
    ...
    success: function (data) {
      toastr.success("Chat Template Saved Succesfully.");
      location.href = "/dashboard/user/openai/chat/ai-chat-list";

      document.getElementById(
        "custom_template_button_user_aichat"
      ).disabled = false;
      document.getElementById("custom_template_button_user_aichat").innerHTML =
        "Save";
    },
...
$(document).ready(function () {
  ...
  let lastInpusGroupId = lastInputsParent
    ? parseInt(lastInputsParent.getAttribute("data-inputs-id"), 10)
    : 0;

  $(".add-more").click(function () {
    const button = this;
    const currentInputs = document.querySelectorAll(
      ".input_name, .input_description, .input_type"
    );
  ...
  $("body").on("click", ".remove-inputs-group", function () {
    ...
    prompt.val(
      currentPromptVal.replaceAll(slugify(parent.attr("data-input-name")), "")
    );
